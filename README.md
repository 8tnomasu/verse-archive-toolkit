...
- 主程式 GUI 內建「路徑與診斷」區，可直接開啟設定檔、日誌與輸出位置，並一鍵複製最近日誌內容
...
- 顯示建庫日誌與完成摘要
- 內建「路徑與診斷」區，可直接開啟設定檔位置、日誌資料夾、輸出資料夾，並一鍵複製最近日誌內容
- 主畫面支援垂直捲動，且摘要 / 日誌區可調整高度
- 可直接開啟翻譯輔助 GUI
...
PowerShell 打包腳本現在會先初始化 UTF-8 輸出設定，並統一使用英文 console 訊息，避免 Windows PowerShell 5.1 常見的中文亂碼問題。
