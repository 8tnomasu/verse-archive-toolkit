...
- 左上日誌區：顯示即時建庫日誌，保留較寬橫向空間，方便閱讀長行訊息與排錯
- 右上可捲動設定區：建庫設定、路徑與診斷
- 下方固定執行區：`儲存設定`、`開始建庫`、`停止 / 取消`、`複製最近日誌內容`、`開啟翻譯工具`、執行狀態、雙來源進度、全域摘要

這樣在建庫期間，大部分操作都可直接在下半部完成；上半部則能同時保留較寬的日誌閱讀空間與較窄但完整可捲動的設定面板。

按下「開始建庫」後，主畫面仍會自動把下方執行區調整到更醒目的高度，讓使用者立刻看到進度與摘要；日誌則固定留在左上區塊持續輸出。
