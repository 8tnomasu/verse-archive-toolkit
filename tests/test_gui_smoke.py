    ...
    def test_builder_window_shows_source_progress_and_path_diagnostics(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with patch.dict(os.environ, {"VERSE_ARCHIVE_TOOLKIT_HOME": str(app_root)}, clear=False):
                ...
                self.assertEqual(window.top_splitter.orientation(), Qt.Horizontal)
                self.assertIs(window.build_splitter.widget(0), window.top_splitter)
                self.assertIs(window.build_splitter.widget(1), window.runtime_panel)
                self.assertIs(window.top_splitter.widget(0), window.log_panel)
                self.assertIs(window.top_splitter.widget(1), window.build_scroll_area)
                self.assertTrue(window.log_panel.isAncestorOf(window.log_output))
                self.assertFalse(window.runtime_panel.isAncestorOf(window.log_output))
                ...
                self.assertTrue(window.runtime_panel.isAncestorOf(window.summary_label))
                build_sizes = window.build_splitter.sizes()
                top_sizes = window.top_splitter.sizes()
                self.assertGreater(build_sizes[0], build_sizes[1])
                self.assertGreater(top_sizes[0], top_sizes[1])
    ...
    def test_start_build_focuses_runtime_panel(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with patch.dict(os.environ, {"VERSE_ARCHIVE_TOOLKIT_HOME": str(app_root)}, clear=False):
                ...
                self.app.processEvents()
                before_sizes = window.build_splitter.sizes()
                ...
                after_sizes = window.build_splitter.sizes()
                self.assertGreater(after_sizes[1], before_sizes[1])
